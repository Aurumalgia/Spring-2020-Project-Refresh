public class RunKT723B {

    public RunKT723B(int rerunNumber){
        int bestRun = 0;
        KnightsTour723B[]kt = new KnightsTour723B[rerunNumber];


        kt[0] = new KnightsTour723B(4, 4);
        while (kt[0].makeMove()==true){
        }
        System.out.println("Run "+0+" moved "+(kt[0].getMoveCounter()-1)+" times. ");

        for(int x = 1; x< kt.length; x++){
            kt[x] = new KnightsTour723B(4, 4);
